    ...
    <div className="flex flex-col h-full w-[400px] glass border-r border-(--border-color) p-10 overflow-hidden shadow-2xl">
      <div className="flex justify-between items-center mb-10">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-(--foreground) font-medium uppercase text-[10px] tracking-[0.3em] mb-4">Signal Intelligence</p>
          </div>
        </div>
        <button 
          onClick={onClose} 
      ...
      <form onSubmit={handleSubmit} className="flex-1 space-y-8 overflow-y-auto pr-2 custom-scrollbar font-roboto">
          <div className="space-y-3">
              <label className="text-[10px] font-black text-(--foreground) uppercase tracking-widest pl-1">Describe the Situation</label>
              <textarea
                  required
                  value={report}
          ...
          <div className="space-y-3">
              <label className="text-[10px] font-black text-(--foreground) uppercase tracking-widest pl-1">Your Contact Profile</label>
              <div className="relative">
                  <Phone className="absolute left-6 top-1/2 -translate-y-1/2 text-(--foreground)" size={18} />
                  <input
                      type="tel"
                      value={phone}
          ...
          <div className="space-y-3">
              <label className="text-[10px] font-black text-(--foreground) uppercase tracking-widest pl-1">Operational Domain</label>
              <div className="flex gap-4 p-1 bg-(--foreground)/5 rounded-2xl border border-(--border-color)">
                  <button 
                    ...
                    className={cn(
                        ...
                        domain === 'human' ? "bg-(--foreground) text-(--background) shadow-lg" : "text-(--foreground) hover:text-(--foreground)"
                    )}
                  >
                      Human Health
                  </button>
                  <button 
                    ...
                    className={cn(
                        "flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all",
                        domain === 'animal' ? "bg-blue-500 text-white shadow-lg" : "text-(--foreground) hover:text-blue-400"
                    )}
                  >
                      Animal Health
                  </button>
