      ...
      <div className="flex items-center gap-6">
        ...
        {!user ? (
          <>
            ...
            <Link href="/signup" className="bg-(--foreground) text-(--background) px-6 py-2.5 rounded-full font-roboto font-black text-xs uppercase tracking-[0.2em] hover:scale-105 active:scale-95 transition-all shadow-[0_10px_20px_var(--border-color)]">
              Join Network
            </Link>
          </>
        ) : (
          <div className="flex items-center gap-4">
            ...
            <button onClick={() => signOut()} className="font-roboto font-bold text-xs uppercase tracking-widest hover:text-emergency transition-colors text-emergency/80">
              Logout
            </button>
          </div>
        )}
