...
const MapInner = (props: LiveMapProps & { L: any }) => {
    ...
    if (
      ...
      try {
        ...
        if (size.x > 0 && size.y > 0) {
          map.flyTo([volunteerLocation.lat, volunteerLocation.lng], 14, {
            duration: 2.5,
          });
  ...
  useEffect(() => {
    if (recenterTrigger && recenterTrigger > 0 && volunteerLocation && map) {
      try {
        map.flyTo([volunteerLocation.lat, volunteerLocation.lng], 16, {
          duration: 1.5,
        });
      } catch {
        /* ignore */
  ...
  return (
    <>
      ...
      {needs
        .filter(
          (n) => n.lat && n.lng && (focusNeed ? n.id === focusNeed.id : false),
        )
        .map((need) => {
          const distance = volunteerLocation
            ? calculateDistance(
                need.lat!,
                need.lng!,
          ...
          return (
            <div key={need.id}>
              ...
              {distance && focusNeed?.id === need.id && (
                <Marker
                  position={[
                    (need.lat! + volunteerLocation!.lat) / 2,
                    (need.lng! + volunteerLocation!.lng) / 2,
                  ]}
                  icon={L.divIcon({
                    className: "distance-label",
                    html: `
                    <div class="bg-slate-900/90 backdrop-blur text-white text-[9px] font-black px-2 py-1 rounded border border-orange-400/50 shadow-lg whitespace-nowrap\">
                      ${formatDistance(distance)} • ${estimateDrivingTime(distance)}
                    </div>
                  `,
                    iconSize: [120, 24],
                    iconAnchor: [60, 12],
        ...
        !isNaN(volunteerLocation.lng) && (
          <>
            ...
            {volunteerLocation.accuracy && (
              <>
                <Circle
                  ...
                  pathOptions={{
                    color:
                      volunteerLocation.accuracy <= 30 ? "#3b82f6" : "#f97316",
                    fillColor:
                      volunteerLocation.accuracy <= 30 ? "#3b82f6" : "#f97316",
                    fillOpacity: 0.1,
                    weight: 2,
                ...
                />
                {volunteerLocation.accuracy > 50 && (
                  <Marker
                    position={[volunteerLocation.lat, volunteerLocation.lng]}
                    icon={L.divIcon({
                      className: "accuracy-warning",
                      html: `
                      <div class="bg-orange-500 text-white text-[8px] font-black px-1.5 py-0.5 rounded border border-orange-300 shadow-lg whitespace-nowrap\">
                        ⚠ Low accuracy ±${Math.round(volunteerLocation.accuracy)}m
                      </div>
                    `,
                      iconSize: [110, 18],
                      iconAnchor: [55, 0],
            ...
            <Marker
              ...
              position={
                [volunteerLocation.lat, volunteerLocation.lng] as [
                  number,
                  number,
                ]
              }
              ...
              <Popup className="custom-popup">
                <div className="p-2 text-center">
                  ...
                  <p className="text-[10px] text-slate-500 mt-1 font-mono">
                    {volunteerLocation.lat.toFixed(5)},{" "}
                    {volunteerLocation.lng.toFixed(5)}
                  </p>
                  {volunteerLocation.accuracy && (
                    <>
                      <p
                      ...
                      <p className="text-[9px] text-slate-600 mt-0.5">
                        Precision: ±{Math.round(volunteerLocation.accuracy)}m
                      </p>
                    </>
                  )}
                </div>
      ...
      {pickedLocation && (
        <Marker
          position={
            [pickedLocation.lat, pickedLocation.lng] as [number, number]
          }
          icon={L.divIcon({
  ...
  if (!isClient || !L)
    return (
      <div className="w-full h-full bg-[var(--background)] flex items-center justify-center rounded-3xl animate-pulse">
        <div className="text-[var(--foreground)] font-anton uppercase tracking-widest text-xl">
    ...
    <div
      className={cn(
        ...
        props.isPickingLocation
          ? "cursor-crosshair ring-4 ring-emergency/30 ring-inset"
          : "",
      )}>
      <MapContainer
      ...
      <div className="absolute bottom-6 left-6 z-[1000] bg-[var(--background)]/80 backdrop-blur-md border border-[var(--border-color)] rounded-2xl p-4 shadow-2xl flex flex-col gap-2.5 pointer-events-auto">
        <p className="text-[10px] font-black text-[var(--foreground)]/50 uppercase tracking-widest mb-0.5">
          Map Legend
        </p>
        ...
        <div className="border-t border-[var(--border-color)] pt-2.5 mt-2.5">
          <p className="text-[9px] font-black text-[var(--foreground)]/50 uppercase tracking-widest mb-1.5">
            Accuracy Status
          </p>
          {props.volunteerLocation?.accuracy && (
            <div
              className={cn(
                "px-2 py-1 rounded text-[8px] font-black uppercase text-center",
                props.volunteerLocation.accuracy <= 30
                  ? "bg-emerald-500/20 text-emerald-600"
                  : props.volunteerLocation.accuracy <= 50
              ...
              {props.volunteerLocation.accuracy <= 30
                ...
                : props.volunteerLocation.accuracy <= 50
                  ? "✓ Good"
                  : "⚠ Lower"}
              <br />±{Math.round(props.volunteerLocation.accuracy)}m
            </div>
          )}
        </div>
      </div>
